...
service / on new http:Listener(SERVICE_PORT) {
    ...
    resource function post questionnaires(QuestionnaireUploadPayload payload) returns string {
        log:printInfo(string `Received questionnaire for file: ${payload.file_name}, job: ${payload.job_id}`);
        string jobKey = payload.file_name + "_" + payload.job_id;
        
        // Replace http://example.org URLs with FHIR server URL in questionnaires
        map<json> processedQuestionnaires = payload.questionnaires;
        if FHIR_SERVER_URL != "" {
            processedQuestionnaires = <map<json>>replaceExampleUrls(processedQuestionnaires.toJson());
        }
        
        lock {
            QUESITONNAIRE_STORE[jobKey] = processedQuestionnaires.cloneReadOnly();
        }
        lock {
        ...
        lock {
            ...
            if metadata is JobMetadata {
                ...
                JOB_METADATA_STORE[jobKey] = metadata;
                log:printInfo(string `Job ${payload.job_id} completed successfully with ${payload.questionnaires.length()} questionnaires`);
            }
        }
        
        // Post questionnaires to FHIR server in background
        if FHIR_SERVER_URL != "" {
            map<json> & readonly questionnairesToPost = processedQuestionnaires.cloneReadOnly();
            _ = start postQuestionnairesToFhirServer(questionnairesToPost);
        }
        ...
        if file_name is string && job_id is string {
            lock {
                ...
                if questionnaires is () {
                    return {"error": "Questionnaires not found for file: " + file_name};
                }
                return questionnaires.cloneReadOnly();
            }
        }
        // Return all questionnaires in memory
        lock {
